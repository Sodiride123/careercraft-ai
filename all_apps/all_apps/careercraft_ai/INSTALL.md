...
- Claude API key (set in claude_wrapper.sh)
...
```bash
cd /workspace/all_apps/careercraft_ai
pip install -r requirements.txt
```

### 2. Configure Claude API
Edit `claude_wrapper.sh` and set your API key:
```bash
...
- `claude_wrapper.sh` - Claude API wrapper script
...
2. Chat with Sarah Chen (AI Career Coach)
...
- Verify Claude API key is correctly set
